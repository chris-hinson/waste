...
pub(crate) fn key_press_handler(
    ...
    mut enemy_monster: 
        Query<(&mut Health, &mut Strength, &mut Defense, &mut Moves, Entity, Option<&Boss>, &Element), 
...
fn calculate_turn(player_stg: &Strength, player_def: &Defense, player_type: &Element, player_action: usize, 
    ...
    if player_action == 2 {
        result.0 = (type_system.type_modifier[*player_type as usize][*enemy_type as usize] * result.0 as f32).trunc() as usize;
    }

    if enemy_action == 2 {
        result.1 = (type_system.type_modifier[*enemy_type as usize][*player_type as usize] * result.1 as f32).trunc() as usize;
    }
