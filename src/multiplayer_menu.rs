                ...
                *color = PRESSED_BUTTON.into();
	            ...
	            match io::stdin().read_line(&mut client_ip_addr) {
		            Ok(_) => {
                         ...
                         //println!("{:?}", split_ip_addr);
                         //ipv4_addr = Ipv4Addr::new(split_ip_addr[0], split_ip_addr[1], split_ip_addr[2], split_ip_addr[3]);
                    }
                    Err(_e) => {
                        // some error handling
                    }
                ...
                // let socket = SocketAddr::new(IpAddr::from(ipv4_addr), client_port.parse().unwrap());
                // println!("{}", socket);
